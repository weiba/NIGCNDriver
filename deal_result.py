...
final_result = pd.read_table(filepath_or_buffer='./data/%s/Cancer_List/result/GCN_NI/final_result.txt' % cancerType, header=0, index_col=0, sep='\t')
# final_result = pd.read_table(filepath_or_buffer='./data/%s/NCG_711/result/GCN_NI/final_result.txt' % cancerType, header=0, index_col=0, sep='\t')
...
# patient_genes.T.to_csv(path_or_buf='./data/%s/NCG_711/result/GCN_NI/sort_gene.txt' % cancerType,sep="\t")
patient_genes.T.to_csv(path_or_buf='./data/%s/Cancer_List/result/GCN_NI/sort_gene.txt' % cancerType,sep="\t")
