...
class TopoSpaceGraphConvolutionLayer(nn.Module, ABC):
    def __init__(self, adj: torch.Tensor, in_dim: int, embed_dim: int, act=torch.relu, bias=False):
    ...
    def forward(self, opposite_x: torch.Tensor):
        ...
        linear_opposite_x = self.lm(opposite_x)
        return collected_opposite_x, linear_opposite_x


class SelfFeature(nn.Module, ABC):
    def __init__(self, adj: torch.Tensor, in_dim: int, embed_dim: int, bias=False, act=torch.relu):
...
class GraphConvolution(nn.Module, ABC):
    def __init__(self, adj: torch.Tensor,  x_feature: torch.Tensor, y_feature: torch.Tensor, mask: torch.Tensor,
        ...
        self.linear1 = nn.Linear(x_feature.size(0), x_feature.size(1))
    ...
    def _add_self_space(self):
        ...
        y = SelfFeature(adj=self.adj.t(), in_dim=self.y.size()[0], embed_dim=self.embed_dim)
        return nn.ModuleList([x, y])

    def _add_topo_space(self):
        x = TopoSpaceGraphConvolutionLayer(adj=self.adj, in_dim=self.y.size()[0], embed_dim=self.embed_dim)
    ...
    def forward(self):
        linear_sample = self.linear1(self.y)
        self_layer_x, self_layer_y = [layer for layer in self.self_space]
        self_x = self_layer_x(x=self.x)
        self_y = self_layer_y(x=linear_sample)

        topo_layer_x, topo_layer_y = [layer for layer in self.topo_space]
        collect_opposite_x, linear_opposite_y = topo_layer_x(opposite_x=linear_sample)
        collect_opposite_y, linear_opposite_x = topo_layer_y(opposite_x=self.x)


        gcn_gene = torch.stack([self_x, collect_opposite_x], dim=0)
        gcn_sample = torch.stack([self_y, collect_opposite_y], dim=0)

        gcn_gene = torch.sum(gcn_gene, dim=0)
        gcn_sample = torch.sum(gcn_sample, dim=0)

        gene_ni = torch.mul(gcn_gene, linear_opposite_x)
        sample_ni = torch.mul(gcn_sample, linear_opposite_y)

        gene_emb = (1-self.gamma)*gcn_gene + self.gamma*gene_ni
        sample_emb = (1-self.gamma)*gcn_sample + self.gamma*sample_ni

        return self.predict_layer(x=gene_emb, y=sample_emb)
